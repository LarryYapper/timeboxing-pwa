...
(function () {
    ...
    async function init() {
        ...
        if (elements.allDayBtn) {
            elements.allDayBtn.addEventListener('click', () => {
                const allDayContainer = document.querySelector('.allday-events');
                if (allDayContainer) {
                    const isHidden = allDayContainer.style.display === 'none';
                    allDayContainer.style.display = isHidden ? 'flex' : 'none';
                    // Force layout recalc after toggle
                    forceImmediateLayout();
                }
            });
        }
